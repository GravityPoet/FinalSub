...
| 视频合成 | 选择视频、字幕、可选配音与输出路径 | 系统 dialog 正常；硬/软字幕结构清楚；软字幕或双音轨自动使用 MKV；视频 metadata 能加载；预览/合成错误可见 |
...
```bash
# 使用应用自带 FFmpeg 生成真实媒体夹具，并验收软字幕+双音轨与硬字幕+智能混音
cd /Users/moonlitpoet/Tools/AI-tools/FinalSub/src-tauri && \
  FINALSUB_COMPOSE_MEDIA_E2E=1 cargo test --lib \
  core::audio::tests::compose_real_media_fixture_when_enabled -- --nocapture
```

| 场景 | 操作 | 通过判据 |
|------|------|----------|
| 既有硬烧回归 | 不添加配音，选择硬字幕 | 生成的参数与引入 compose 前逐参数一致 |
| 软字幕 MKV | 选择软字幕，填写语言与轨道名称 | 视频/原声均 stream copy；MKV 含 1 条可开关字幕轨及正确 metadata |
| 替换原声 | 添加配音并选择“替换原声” | 成品只含配音；视频仅在硬字幕时重编码 |
| 智能混音 | 添加配音并选择“智能混音” | `sidechaincompress` 在配音出声时压低原声，再合成为单音轨；无原声音轨时 UI 禁用 |
| 双音轨 | 添加配音并选择“双音轨” | 自动使用 MKV；原音轨 stream copy，新增配音单独编码 AAC，并可在播放器切换 |
| 内置样式完整应用 | 在字幕合成页依次选择 6 套内置样式 | 字体、字号、前景/描边/背景色、描边、阴影、背景模式、位置与边距整组切换，画布立即预览 |
| 个人样式生命周期 | 调整样式后保存两项，更新、上移/下移、尝试重复名并删除一项 | 成功状态明确、顺序持久；重复名被拦截；删除提示说明历史任务使用快照且不受影响 |
| 新任务复用 | 新建任务开启“成品视频”，选择个人样式，再切到软字幕后切回硬字幕 | 个人样式可选；往返不丢选择；创建请求携带完整样式快照，之后修改预设不改变任务 |
| 响应式 | 1200×800 与 390×844 切换硬/软字幕及音轨模式 | 无横向溢出；软字幕时隐藏无效的硬字幕样式区；控制台 0 error |
