          ...
          {step === 'config' && (
            <div className="space-y-5">
              <div className="space-y-2">
                <label className="text-xs font-semibold text-slate-400">{t('proofread.batchAi.selectService')}</label>
              ...
              <div className="space-y-2">
                <label className="text-xs font-semibold text-slate-400">{t('proofread.batchAi.batchSize')}</label>
              ...
              <div className="p-4 bg-slate-950/40 border border-slate-800 rounded-xl flex items-center justify-between">
                <div>
                  <span className="text-xs text-slate-400">{t('proofread.batchAi.totalSubs')}</span>
                  <div className="text-xl font-bold text-slate-200 mt-1">
                    {t('proofread.batchAi.totalSubsCount').replace('{count}', String(subtitles.filter((s) => s.sourceContent?.trim()).length))}
                  </div>
                </div>
                <div className="text-right">
                  <span className="text-xs text-slate-400">{t('proofread.batchAi.expectedBatches')}</span>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <label className="text-xs font-semibold text-slate-400">{t('proofread.batchAi.customPrompt')}</label>
          ...
          {step === 'running' && (
            <div className="py-12 flex flex-col items-center justify-center space-y-6">
              ...
              <div className="text-center space-y-1.5">
                <p className="text-base font-semibold">{t('proofread.batchAi.optimizing')}</p>
                          ...
                          result.optimizedTarget !== result.originalTarget && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                              <div className="p-3 bg-slate-900/60 rounded-xl text-xs border border-slate-800">
                                <div className="text-[10px] text-slate-500 mb-1.5">{t('proofread.batchAi.originalTarget')}</div>
                                <div className="text-slate-300 font-medium">{result.originalTarget || t('proofread.batchAi.emptyText')}</div>
                              </div>
                              <div className="p-3 bg-emerald-950/20 rounded-xl text-xs border border-emerald-900/20">
                                <div className="text-[10px] text-emerald-500 mb-1.5">{t('proofread.batchAi.optimizedTarget')}</div>
