...
import { fsReadDir, fsExists } from '../../lib/tauri';
import {
  DetectedSubtitle,
...
// 简易 pathShim，避免前端环境引入 Node.js path 模块
export const pathShim = {
  ...
  dirname: (filePath: string) => {
    ...
    if (filePath.startsWith('/') && !joined.startsWith('/')) {
      return '/' + joined;
    }
    return joined || '.';
  },
  join: (...args: string[]) => {
    const parts = args.map(part => part.replace(/\\/g, '/'));
...
export async function detectSubtitlesForVideo(
  ...
  const directory = pathShim.dirname(videoPath);
  const videoName = pathShim.basename(videoPath, pathShim.extname(videoPath));

  // 获取目录下所有字幕文件
  let files: string[] = [];
  try {
    const allFiles = await fsReadDir(directory);
    files = allFiles.filter((f: string) => isSubtitleExtension(f));
  } catch (error) {
  ...
  for (const file of files) {
    const filePath = pathShim.join(directory, file);
...
function analyzeSubtitleFile(
  ...
  const fileName = pathShim.basename(filePath, pathShim.extname(filePath));
...
function extractBaseName(fileName: string): string {
  const name = pathShim.basename(fileName, pathShim.extname(fileName));
...
export async function matchSubtitlesByRules(
  ...
  for (const [key, groupFiles] of Array.from(fileGroups.entries())) {
    const match: SubtitleMatchResult = {
      baseName: pathShim.basename(key),
    };
    ...
    if (!match.source && !match.target) {
      for (const file of groupFiles) {
        const fileName = pathShim.basename(file, pathShim.extname(file)).toLowerCase();
...
export async function scanDirectoryForSubtitles(
  ...
  try {
    ...
    for (const file of files) {
      const filePath = pathShim.join(directoryPath, file);
      if (isSubtitleExtension(file)) {
...
export async function validateSubtitleFile(filePath: string): Promise<boolean> {
  try {
    ...
    return exists && isSubtitleExtension(filePath);
  } catch {
    return false;
  }
}
