...
- SmartSub 上游审计已更新到 `dd38b8aecd8934b7218b8973131a88fd9826c208`；逐项核对目标驱动任务向导、持久人工闸门、dubbing/compose、配方、阶段产物、AI thinking control、字幕样式预设、云声音复刻与 TTS worker。FinalSub 已补齐统一阶段编排、双审核闸门、批准后自动续跑、交付目标向导、服务商感知的显式思考控制、可复用样式预设、ElevenLabs IVC、豆包声音复刻主链路，以及语言感知估时、会话校准、合成前预控、一次本地重合成、云端不重复计费的后处理、1.5× 持久人工闸门、独立 TTS worker 崩溃隔离和任务内 1–3 路本地多进程并行。当前差距项为真实本地模型音质与付费账号 E2E、跨平台真实安装验收。
...
- 全量 `cargo test --lib` 为 324 passed / 0 failed / 8 ignored（332 项）；独立进程集成测试另有 3/3 通过，覆盖三 worker PID 隔离、协议握手、畸形/超大消息恢复、强杀不带崩父进程与替代进程重启。其余新增覆盖混合媒体/字幕发现、配对字幕任务持久化、跳过 ASR、语言感知估时、四档预控、一次本地重合成、云端单次计费、服务商速度边界、1.5× 红线、校准边界、人工处理续跑与真实 FFmpeg WAV 时长收敛，并保留统一存储根目录跟随、逐引擎覆盖、Unicode 路径和已有 Parakeet 目录无需下载的扫描闭环。`cargo fmt --check`、`git diff --check` 与 `npm run build` 通过。
...
- 本轮 Universal 1.0.10 已原子安装到唯一 `/Applications/FinalSub.app`；主程序、FFmpeg、Whisper 均为 `x86_64 arm64`，deep strict 签名、真实运行路径与构建目录残留 `.app` 为 0 均通过，当前 GUI PID 为 `96413`。最新 DMG SHA-256 `de4e631e43eb06cf807b74b3d4de5b32e9378dff70d24eb5c5e2c126ed4b8aae`；本轮安装前回滚 ZIP 位于 `~/Library/Application Support/FinalSub/Backups/20260721-054629/FinalSub.app.zip`。
