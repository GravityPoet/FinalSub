...
- 每个配置实例提供“测试连接”；先保存当前草稿，再由 Rust 从 endpoint 隔离的凭据库取密钥并向真实转写接口上传 1 秒静音 WAV，前端不接触明文密钥，测试不重试以避免重复用量。
...
| 云 TTS | 🟡 | OpenAI-compatible、Azure Speech、ElevenLabs、火山引擎豆包语音 V3 与 Edge TTS 免费试用档已接入；Edge 已真实合成中文 24 kHz PCM WAV，并由本机 Whisper large-v3-turbo 回听完整命中正文；付费账号仍缺真实 E2E，Edge 无需 Key 但不承诺稳定性 |
...
| 引擎进程隔离 | 🟢 | 本地 TTS 复用 FinalSub 自身二进制的私有 worker 模式，按任务选择 1–3 路独立 PID 并在子进程内缓存模型；主进程只通过 64 KB 上限的逐行 JSON 协议通信，启动先做版本/PID 握手。取消只终止对应 worker 并清理临时 WAV，崩溃只让当前行失败、下一次请求自动重建；批量结束回收额外 worker，删除/切换模型目录与安装更新前停止全部 worker。真实进程测试已验证三进程 PID 隔离、畸形消息恢复、强杀不带崩父进程和替代 worker 重启 |
...
| 任务向导 | 🟢 | 首屏直接展示素材、识别核心、任务概览与开始入口；首次引导缩为右上角非阻塞卡片。先选字幕、配音和最终视频交付目标，再按目标显示 TTS、音轨、字幕封装和编码配置；已选媒体、字幕与配对关系跨任务类型切换保留，类型不匹配时可一键切回兼容流程且不重新上传；全部配对后隐藏模型下载前置条件 |
| 配方 | 🟢 | 4 个内置配方（含“视频 → 配音成片”）+ 用户配方；目标、审核闸门、TTS、compose 与完整字幕样式快照随配方保存，失效模型引用安全回退到当前可用项，旧配方缺失样式字段时安全使用默认值 |
| 人工闸门 | 🟢 | 字幕校对和配音确认均为持久阶段；批准后自动把下一节点置为 pending 并启动 worker，不重做 ASR、不重新上传媒体，也不覆盖已校对字幕。严重超长配音会强制停在当前配音节点，处理后重跑该节点完成音轨导出，不会把未生成音轨误标为完成 |
...
| 响应式 | 🟢 | 桌面侧栏、移动底栏；任务概览仅在 1440px 以上进入右栏，避免 1200px 窗口把来源区压窄；1200×575/800 与 390×844 无页面横向溢出，矮窗口侧栏可独立滚动且不覆盖快速命令 |
| 导航效率 | 🟢 | `⌘K` 根级命令面板、`⌘1`–`⌘9`、活动中心、首次引导和可折叠侧栏；选中态使用导轨而非通知圆点 |
| 国际化 | 🟢 | 中文、英文、日文各 1,449 个 key 完全对齐，duplicate 0；首次启动及自动模式按系统语言列表选择，无法匹配时回退英文，并支持手动覆盖 |
| 路由体积 | 🟢 | 页面 lazy loading；生产构建按页面拆包 |
| 浏览器 QA | 🟢 | 本地/云端模型分区、本地 TTS 路径、目标向导、配音/compose 配置、双审核闸门与批准续跑均已实测；切换“仅生成字幕 / 生成并翻译 / 仅翻译”不会清空已选媒体与字幕配对，且可直接回到兼容流程继续；自动配对、手动 ASR 回退、模型前置条件显隐与首次引导关闭均已实测。新建任务与配音工作台的 1/2/3 路选择器、3 路批量完成与超长复核状态已实测；配音估时/首轮/最终速度、“自动重合成 / 等待人工处理”标签及任务会话直达已实测；样式完整应用、个人预设保存/更新/重排/删除、重复名拦截、新建任务复用及硬软字幕往返保留选择均已实测；1200×800 与 390×844 无横向溢出 |
...
- 免费翻译真实公网 smoke：无需密钥的自动入口返回非空中文译文；本地模拟服务验证 Bing 503 后自动切换 Google，公共源字符上限、节流和错误聚合均有测试。
- TTS 听感样本：`zh-CN-XiaoxiaoNeural` 真实合成 10.70 秒中文，24 kHz / mono / PCM s16le，峰值 -4.28 dB、RMS -19.71 dB、无削波；本机 Whisper large-v3-turbo 回听完整命中两句正文，仅重建标点与空格。
...
- 本轮 Universal 1.0.10 已原子安装到唯一 `/Applications/FinalSub.app`；主程序、FFmpeg、Whisper 均为 `x86_64 arm64`，deep strict 签名、真实运行路径与构建目录残留 `.app` 为 0 均通过，当前 GUI PID 为 `96586`。最新 DMG SHA-256 `4f98d518fe4b600bf9f0531ee11154528ab1a8682cab4fb62152b952aa1f2b2c`；本轮安装前回滚 ZIP 位于 `~/Library/Application Support/FinalSub/Backups/20260722-052702/FinalSub.app.zip`。
